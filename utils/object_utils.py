...
from utils.config import VELOCITY_THRESHOLD, RAW_DATA_FORMAT, OBJ_RADIUS, EXIST_THRESHOLD, OBS_LEN, PRED_LEN, NORMALIZED_CENTER
...
def get_nearby_moving_obj_feature_ls(agent_df, traj_df, obs_len, seq_ts, norm_center):
    ...
    for track_id, remain_df in traj_df.groupby('TRACK_ID'):
        if remain_df['OBJECT_TYPE'].iloc[0] == 'AGENT':
            continue
        elif remain_df['OBJECT_TYPE'].iloc[0] == 'AV':
            continue
    ...
    return obj_feature_ls   # 在第一个场景中, 总共有25个object


def get_non_tgt_agent_feature_ls(agent_df: pd.DataFrame, 
                                 length_threshold: int = EXIST_THRESHOLD,
                                 ):
                                #  -> List[np.ndarray, str, np.ndarray, str]
    """
    Args:
        - agent_df
        - length_threshold: threshold for track length

    Return:
        If the track is long enough, return a list of [double track, object type, ts, track id], otherwise return None.
        xyz: shape = [len(agent_df)-1, 4]
        ts: shape = [len(agent_df) - 1]
    """
    
    if len(agent_df) < length_threshold:
        return None
    xys = agent_df[["X", "Y"]].values
    ...
    return [xys, 'non-tgt', ts, agent_df['TRACK_ID'].iloc[0]]