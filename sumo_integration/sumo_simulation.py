...
SumoActor = collections.namedtuple('SumoActor', 'type_id vclass transform signals extent color velocity')
...
class SumoSimulation(object):
    ...
    def __init__(self, cfg_file, step_length, host=None, port=None, sumo_gui=False, client_order=1):
        ...
        self.model.eval()
        self.trajs = {}
        self.yaw_dict = pkl.load(open("sumo_files/sumocfg/yaw_dict_10m.pkl", "rb"))
        self.control_ids = []
        self.odometries = []
        self.vehicle_id_last_step = tuple()
    ...
    def get_actor(actor_id):
        ...
        return SumoActor(type_id, vclass, transform, signals, extent, color, velocity)
    ...
    def tick(self):
        ...
        remove_vehicle_ids = []
        for v in self.vehicle_id_last_step:
            if v not in vehicle_ids:
                remove_vehicle_ids.append(v)
        time = traci.simulation.getTime()
        self.sumo_time = time
        # print('time',time)
        # print('vehicle_ids: ', vehicle_ids)
        # print('trajs: ', trajs.keys())
        trajs = update_trajs(vehicle_ids, time, self.trajs, self.yaw_dict)
            ...
            for tgt in range(num_tgt_agent):
                ...
                if dist < 30:
                    ...
                    try:
                        ...
                        pdb.set_trace()
                elif (dist > 38) and (vehicle_id in self.control_ids):
                    odometry = traci.vehicle.getDistance(vehicle_id)
                    self.odometries.append(odometry - 60)
                    self.control_ids.remove(vehicle_id)
        ...
        self.spawned_actors = set(traci.simulation.getDepartedIDList())
        self.destroyed_actors = set(traci.simulation.getArrivedIDList())
        for v in remove_vehicle_ids:
            if v not in self.destroyed_actors:
                self.destroyed_actors.add(v)
        self.vehicle_id_last_step = vehicle_ids
        remove_vehicles(vehicle_ids, trajs)
